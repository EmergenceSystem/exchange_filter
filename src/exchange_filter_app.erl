...
start(_Type, _Args) ->
    case exchange_filter_sup:start_link() of
        {ok, Pid} ->
...
stop(_State) ->
    catch cowboy:stop_listener(exchange_filter_query_listener),
    catch em_pop_sup:stop_node(exchange_filter),
    ok.
...
start_pop_and_http() ->
    PopPort   = application:get_env(exchange_filter, pop_port,   9432),
    QueryPort = application:get_env(exchange_filter, query_port, 9433),
    Seeds     = application:get_env(exchange_filter, pop_seeds,  []),
    Vec = em_filter_vec:from_capabilities(base_capabilities()),
    catch em_pop_sup:stop_node(exchange_filter),
    catch cowboy:stop_listener(exchange_filter_query_listener),
    {ok, PopPid} = em_pop_sup:start_node(exchange_filter, #{
        port            => PopPort,
    ...
    Dispatch = cowboy_router:compile([
        {'_', [{"/agent/query", em_filter_http,
                #{server => exchange_filter_server}}]}
    ]),
    {ok, _} = cowboy:start_clear(exchange_filter_query_listener,
                                  [{port, QueryPort}],
                                  #{env => #{dispatch => Dispatch}}),
    logger:notice("[exchange_filter] gossip port ~w  query port ~w",
                  [PopPort, QueryPort]),
    ok.
